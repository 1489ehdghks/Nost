...
const BookRating = ({ bookId, initialRating, onRatingChange }) => {
  ...
  useEffect(() => {
    const fetchUserRating = async () => {
      try {
        const response = await axiosInstance.get(`https://nost-stella.com/api/books/${bookId}/rating/`);
  ...
  const rateBook = async (newRating) => {
    try {
      const response = await axiosInstance.post(`https://nost-stella.com/api/books/${bookId}/rating/`, { rating: newRating });
      const { rating } = response.data;
      setRating(rating);
      onRatingChange(rating);
    } catch (error) {
